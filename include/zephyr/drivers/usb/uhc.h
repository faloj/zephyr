...
/**
 * @brief USB host controller transfer states
 */
enum uhc_xfer_state_type {
	XFER_IDLE = 0u,
	XFER_DONE = 1,
	XFER_NOTREADY = 2,
	XFER_NYET = 3,
...
struct uhc_transfer {
	...
	void *owner;
	/** Transfer state type */
	volatile enum uhc_xfer_state_type xfer_state;
};
...
struct uhc_api {
	...
	int (*pipe_open)(const struct device *dev,
			  uint8_t pipe_num,
			  uint8_t pipe_dir,
			  uint8_t pipe_type,
			  uint16_t pipe_mps);
 ...
 * @retval -EPERM controller is not initialized
 */
int uhc_pipe_open(const struct device *dev,
		   uint8_t pipe_num,
		   uint8_t ep_num,
		   uint8_t ep_type,
		   uint16_t ep_mps);
