...
USBH_CONTROLLER_DEFINE(stm32_otg_ctx, DEVICE_DT_GET(DT_DRV_INST(0)));
...
#define DWC_HOST_TIMEOUT_VAL		  (500000u)
...
static uint32_t uhc_stm32_get_speed(mem_addr_t base_addr)
{
	uint32_t reg_val = 0;
...
/* halt channel */
static int uhc_stm32_hch_halt(struct dev_priv_data *priv, uint32_t ch_num)
{
...
static int uhc_stm32_hc_init(mem_addr_t base_addr, uint8_t ch_num, uint8_t epnum, uint8_t dev_address,
		       uint8_t speed, uint8_t ep_type, uint16_t mps)
{
	...
	core_speed = uhc_stm32_get_speed(base_addr);
...
static int uhc_stm32_open_pipe(struct dev_priv_data *priv, uint8_t ch_num, uint8_t epnum,
			 uint8_t dev_address, uint8_t speed, uint8_t ep_type, uint16_t mps)
{
	...
	ret_val = uhc_stm32_hc_init(priv->base_addr, ch_num, epnum, dev_address, speed, ep_type, mps);

	return ret_val;
}

/* initiate USB transfer */
static int uhc_stm32_start_transfer(mem_addr_t base_addr, struct host_ch_mng *p_hch, uint8_t dma)
{
	...
	if (p_hch->ep_is_in) {
		p_hch->xfer_sz = (uint32_t)num_packets * p_hch->max_packet;
		sys_cache_data_invd_range((void *)p_hch->xfer_buff, p_hch->xfer_len);
	} else {
		p_hch->xfer_sz = p_hch->xfer_len;
		sys_cache_data_flush_range((void *)p_hch->xfer_buff, p_hch->xfer_len);
	}
...
static int uhc_stm32_submit_transfer_req(struct dev_priv_data *priv, uint8_t ch_num, bool data_in,
				   uint8_t ep_type, uint8_t token, uint8_t *p_buf, uint16_t length)
{
	...
	priv->hch[ch_num].urb_state = DCW_URB_IDLE;
	priv->hch[ch_num].ch_num = ch_num;
	priv->hch[ch_num].state = DWC_HC_IDLE;

	return uhc_stm32_start_transfer(priv->base_addr, &priv->hch[ch_num], 1);
}

/* send a setup packet */
static int uhc_stm32_ctrl_send_setup(struct dev_priv_data *priv, uint8_t *buf, uint8_t pipe_num)
{
	int ret_val = 0;

	ret_val = uhc_stm32_submit_transfer_req(priv, pipe_num, false, DWC_EP_TYPE_CTRL, DWC_PID_SETUP,
					  buf, DWC_SETUP_PKT_SIZE);
	return ret_val;
}

/* control transfer receive data*/
static int uhc_stm32_ctrl_receive_data(struct dev_priv_data *priv, uint8_t *buf, uint16_t length,
				 uint8_t pipe_num)
{
	return uhc_stm32_submit_transfer_req(priv, pipe_num, true, DWC_EP_TYPE_CTRL, DWC_PID_DATA, buf,
					  length);
}

/* control transfer send data */
static int uhc_stm32_ctrl_send_data(struct dev_priv_data *priv, uint8_t *buf, uint16_t length,
			      uint8_t pipe_num)
{
	return uhc_stm32_submit_transfer_req(priv, pipe_num, false, DWC_EP_TYPE_CTRL, DWC_PID_DATA,
					  buf, length);
}

/* bulk transfer receive data */
static int uhc_stm32_bulk_receive_data(struct dev_priv_data *priv, uint8_t *buf, uint16_t length,
				 uint8_t pipe_num)
{
	return uhc_stm32_submit_transfer_req(priv, pipe_num, true, DWC_EP_TYPE_BULK, DWC_PID_DATA, buf,
					  length);
}

/* bulk transfer send data */
static int uhc_stm32_bulk_send_data(struct dev_priv_data *priv, uint8_t *buf, uint16_t length,
			      uint8_t pipe_num)
{
	return uhc_stm32_submit_transfer_req(priv, pipe_num, false, DWC_EP_TYPE_BULK, DWC_PID_DATA,
					  buf, length);
}

/* manage pipes based on device properties */
static int uhc_stm32_pipe_mngmt(struct dev_priv_data *priv)
{
	struct pipe_mng *pipe_info = &(priv->pipe_info);

	switch (pipe_info->request) {
	case USB_SREQ_SET_ADDRESS:
		(void)uhc_stm32_open_pipe(priv, DWC_DEFAULT_CTRL_PIPE_OUT, USB_EP_DIR_OUT,
			pipe_info->addr, DWC_DEVICE_SPEED_HIGH, DWC_EP_TYPE_CTRL, DWC_DEFAULT_MPS);
		(void)uhc_stm32_open_pipe(priv, DWC_DEFAULT_CTRL_PIPE_IN, USB_EP_DIR_IN,
			pipe_info->addr, DWC_DEVICE_SPEED_HIGH, DWC_EP_TYPE_CTRL, DWC_DEFAULT_MPS);
		break;
	default:
		break;
...
static int uhc_stm32_xfer_control(const struct device *dev, struct uhc_transfer *const xfer)
{
	...
	if (!xfer->setup) {
		...
		(void)uhc_stm32_ctrl_send_setup(priv, buf->data, DWC_DEFAULT_CTRL_PIPE_OUT);
		priv->schedule = true;
		uhc_xfer_setup(xfer);
	...
	if (buf->size != 0) {
		...
		if (USB_EP_DIR_IS_IN(xfer->ep)) {
			...
			(void)uhc_stm32_ctrl_receive_data(priv, data, length, DWC_DEFAULT_CTRL_PIPE_IN);
		} else {
			length = MIN(buf->len, xfer->mps);
			data = buf->data;
			(void)uhc_stm32_ctrl_send_data(priv, data, length, DWC_DEFAULT_CTRL_PIPE_OUT);
		}
		...
		/* status stage */
		if (USB_EP_DIR_IS_IN(xfer->ep)) {
			(void)uhc_stm32_ctrl_send_data(priv, NULL, 0, DWC_DEFAULT_CTRL_PIPE_OUT);
		} else {
			(void)uhc_stm32_ctrl_receive_data(priv, NULL, 0, DWC_DEFAULT_CTRL_PIPE_IN);
		}

		if (priv->pipe_info.data_change) {
			priv->pipe_info.data_change = false;
			(void)uhc_stm32_pipe_mngmt(priv);
		}
	}
...
static int uhc_stm32_xfer_bulk(const struct device *dev, struct uhc_transfer *const xfer)
{
	...
	if (USB_EP_DIR_IS_IN(xfer->ep)) {
		...
		data = net_buf_tail(buf);
		(void)uhc_stm32_bulk_receive_data(priv, data, length, DWC_DEFAULT_BULK_PIPE_IN);
	} else {
		length = buf->len;
		data = buf->data;
		(void)uhc_stm32_bulk_send_data(priv, data, length, DWC_DEFAULT_BULK_PIPE_OUT);
	}
...
static int uhc_stm32_schedule_xfer(const struct device *dev)
{
	...
	if (USB_EP_GET_IDX(priv->last_xfer->ep) == 0) {
		...
		return uhc_stm32_xfer_control(dev, priv->last_xfer);
	}

	return uhc_stm32_xfer_bulk(dev, priv->last_xfer);
}

/* handle transfer success event */
static int uhc_stm32_handle_xfr_success(const struct device *dev)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
...
static void uhc_stm32_xfer_drop_active(const struct device *dev, int err)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
...
static int uhc_stm32_handle_xfrc(const struct device *dev)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
	struct uhc_transfer *const xfer = priv->last_xfer;
	int ret;
	...
	if (!uhc_xfer_is_queued(xfer)) {
		...
		uhc_stm32_xfer_drop_active(dev, -EINVAL);
		return -EINVAL;
	}
	...
	if (k_fifo_is_empty(&xfer->queue)) {
		LOG_ERR("No buffers to handle");
		uhc_stm32_xfer_drop_active(dev, -ENODATA);
		return -ENODATA;
	}
	...
	if (ret || (priv->curr_state != DCW_URB_DONE)) {
		LOG_ERR("Transfer timed out");
		uhc_stm32_xfer_drop_active(dev, -ETIMEDOUT);
		return -ETIMEDOUT;
	}
	xfer->xfer_state = priv->curr_state;
	priv->curr_state = DCW_URB_IDLE;

	ret = uhc_stm32_handle_xfr_success(dev);
	if (ret) {
		uhc_stm32_xfer_drop_active(dev, ret);
		return 0;
	}
...
/* thread to handle transfers */
static void uhc_stm32_thread(const struct device *dev)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
	int err;
	uint8_t count = 0;
	...
	while (true) {
		...
		while ((count < total_stages) && (priv->schedule)) {
			...
			err = uhc_stm32_schedule_xfer(dev);
			if (unlikely(err)) {
				uhc_submit_event(dev, UHC_EVT_ERROR, err, NULL);
			}

			err = uhc_stm32_handle_xfrc(dev);
			if (err) {
				priv->schedule = false;
			}
...
/* enqueue a transfer */
static int uhc_stm32_enqueue(const struct device *dev, struct uhc_transfer *const xfer)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
...
/* dequeue a transfer */
static int uhc_stm32_dequeue(const struct device *dev, struct uhc_transfer *const xfer)
{
	/* TODO */
	return 0;
}
...
static int uhc_stm32_pipe_open(const struct device *dev, uint8_t pipe_num, uint8_t ep_num,
			     uint8_t ep_type, uint16_t ep_mps)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
	int ret_val =
		uhc_stm32_open_pipe(priv, pipe_num, ep_num, 1, DWC_DEVICE_SPEED_HIGH, ep_type, ep_mps);
...
/* perform core reset */
static int uhc_stm32_core_reset(mem_addr_t base_addr)
{
	uint32_t reg_val = 0;
...
/* flush tx FIFO */
static int uhc_stm32_flush_tx_fifo(mem_addr_t base_addr, uint8_t num)
{
	uint32_t reg_val = 0;
...
/* flush rx FIFO */
static int uhc_stm32_flush_rx_fifo(mem_addr_t base_addr)
{
	uint32_t reg_val = 0;
...
/* initialize registers for host mode */
static int uhc_stm32_reg_init(struct dev_priv_data *priv)
{
	uint32_t i = 0;
	int ret_val = 0;
	uint32_t reg_val = 0;
	...
	/* enable ULPI interface */
	reg_val = sys_read32(priv->base_addr + DWC_GUSBCFG_OFFSET);
	reg_val |= BIT(DWC_GUSBCFG_PHYSEL);
	sys_write32(reg_val, priv->base_addr + DWC_GUSBCFG_OFFSET);
#endif
	ret_val = uhc_stm32_core_reset(priv->base_addr);
	if (ret_val) {
		return ret_val;
	}
#if 1
	...
	/* flush FIFOs */
	ret_val = uhc_stm32_flush_tx_fifo(priv->base_addr, DWC_GRSTCTL_FLUSH_ALL_TXFIFO);
	if (ret_val) {
		return ret_val;
	}

	ret_val = uhc_stm32_flush_rx_fifo(priv->base_addr);
	if (ret_val) {
		return ret_val;
	}
...
static int uhc_configure_vbus(const struct device *dev, mem_addr_t base_addr, bool enable_flag)
{
	const struct uhc_cfg *config = dev->config;
	...
	if (config->vbus_enable_gpio.port) {
		...
		if (enable_flag) {
			err = gpio_pin_set_dt(&config->vbus_enable_gpio, 1);
		} else {
...
/* reset host */
static int uhc_stm32_reset_host(mem_addr_t base_addr)
{
	uint32_t i;
	int ret_val = 0;
	uint32_t reg_val = 0;
	...
	/* flush FIFOs */
	ret_val = uhc_stm32_flush_tx_fifo(base_addr, DWC_GRSTCTL_FLUSH_ALL_TXFIFO);
	if (ret_val) {
		return ret_val;
	}

	ret_val = uhc_stm32_flush_rx_fifo(base_addr);
	if (ret_val) {
		return ret_val;
	}
...
static int uhc_isr_channel_in_handler(struct dev_priv_data *priv, uint32_t ch_num)
{
	...
	if (DWC_INTERRUPT_CHECK_SOURCE(reg_val, BIT(DWC_HCINTx_CHH))) {
		if (DWC_INTERRUPT_CHECK_SOURCE(reg_val, BIT(DWC_HCINTx_XFRC))) {
			...
			uhc_stm32_hch_halt(priv, ch_num);
			priv->hch[ch_num].state = DWC_HC_XFRC;
		...
		if (priv->hch[ch_num].state == DWC_HC_XFRC) {
			priv->curr_state = DCW_URB_DONE;
...
static void uhc_stm32_isr_handler(struct device *dev)
{
	struct dev_priv_data *priv = uhc_get_private(dev);
	mem_addr_t base_addr = priv->base_addr;
	uint32_t reg_val = 0;
 ...
 */
static int uhc_stm32_reset_config(const struct reset_dt_spec *reset_spec)
{
	int ret = 0;
...
static int uhc_stm32_init(const struct device *dev)
{
	...
	ret_val = pinctrl_apply_state(config->pcfg, PINCTRL_STATE_DEFAULT);

	if (ret_val != 0) {
		LOG_ERR("USB pinctrl setup failed (%d)", ret_val);
		return ret_val;
	}
	...
	if (config->vbus_enable_gpio.port) {
		...
		ret_val = gpio_pin_configure_dt(&config->vbus_enable_gpio, GPIO_OUTPUT_INACTIVE);
		if (ret_val != 0) {
			LOG_ERR("Failed to configure vbus enable gpio (%d)", ret_val);
			return ret_val;
		}
	}

	ret_val = priv_clock_enable(dev);
	...
	ret_val = uhc_stm32_reg_init(priv);
	if (ret_val) {
		LOG_ERR("USB register initialization failed (%d)", ret_val);
		return ret_val;
	}

	/* configure vbus */
	uhc_configure_vbus(dev, priv->base_addr, true);

	/* configure ISR handler and enable irq */
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority), uhc_stm32_isr_handler,
...
/* reset port */
static int uhc_stm32_reset_port(struct dev_priv_data *priv)
{
	int ret_val = 0;
	uint8_t reset_cnt = 0;
	mem_addr_t base_addr = priv->base_addr;
...
/* enable core to be ready for enumeration process */
static int uhc_stm32_enable(const struct device *dev)
{
	...
	if (priv->device_connected) {
		ret_val = uhc_stm32_reset_port(priv);

		/* open control transfer pipes required for enumeration */
		if (!ret_val) {
			ret_val = uhc_stm32_open_pipe(priv, DWC_DEFAULT_CTRL_PIPE_OUT, 0x00,
						DWC_DEFAULT_ADDRESS, DWC_DEVICE_SPEED_FULL,
						DWC_EP_TYPE_CTRL, DWC_DEFAULT_MPS);
			if (!ret_val) {
				ret_val = uhc_stm32_open_pipe(priv, DWC_DEFAULT_CTRL_PIPE_IN, 0x80,
							DWC_DEFAULT_ADDRESS, DWC_DEVICE_SPEED_FULL,
							DWC_EP_TYPE_CTRL, DWC_DEFAULT_MPS);
				if (ret_val) {
...
static int uhc_stm32_shutdown(const struct device *dev)
{
	...
	uhc_configure_vbus(dev, priv->base_addr, false);
...
static int uhc_stm32_driver_preinit(const struct device *dev)
{
	...
	k_thread_create(&drv_stack_data, drv_stack, K_KERNEL_STACK_SIZEOF(drv_stack),
			(k_thread_entry_t)uhc_stm32_thread, (void *)dev, NULL, NULL, K_PRIO_COOP(2), 0,
			K_NO_WAIT);
	k_thread_name_set(&drv_stack_data, "uhc_stm32");

	/* reset USB */
	if (cfg->reset_spec.dev != NULL) {
		ret = uhc_stm32_reset_config(&(cfg->reset_spec));
		if (ret != 0) {
			return ret;
		}
	}
	...
	LOG_DBG("UHC STM32 driver pre-initialized");
...
static const struct uhc_api uhc_stm32_api = {
	...
	.ep_enqueue = uhc_stm32_enqueue,
	.ep_dequeue = uhc_stm32_dequeue,
	.pipe_open = uhc_stm32_pipe_open,
};

static struct dev_priv_data uhc_stm32_priv = {
	.xfr_sem = Z_SEM_INITIALIZER(uhc_stm32_priv.xfr_sem, 0, 1),
	.xfr_event_sem = Z_SEM_INITIALIZER(uhc_stm32_priv.xfr_event_sem, 0, 1),
};

static struct uhc_data uhc_stm32_data = {
	.priv = &uhc_stm32_priv,
};

PINCTRL_DT_INST_DEFINE(0);

struct uhc_cfg uhc_stm32_cfg = {
...
DEVICE_DT_INST_DEFINE(0, uhc_stm32_driver_preinit, NULL, &uhc_stm32_data, &uhc_stm32_cfg, POST_KERNEL,
